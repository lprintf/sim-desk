...
/** Ask the Node bridge to start or re-check the Codex App Server. */
function LaunchIdeButton() {
  ...
  const handleLaunch = useCallback(async () => {
    ...
    setState('launching');
    try {
      const response = await fetch(`${API_BASE}/api/codex/status`, { headers: authHeaders() });
      if (!response.ok) throw new Error('Codex App Server is unavailable');
      setState('launched');
  ...
  return (
    ...
    >
      {state === 'launched' ? (
        <><Check className="w-4 h-4" /> Codex is ready</>
      ) : state === 'launching' ? (
        <><Loader2 className="w-4 h-4 animate-spin" /> Connecting...</>
      ) : (
        <><Rocket className="w-4 h-4" /> Reconnect Codex</>
      )}
    </button>
  );
}
        ...
        <div className="flex flex-col flex-1 min-w-0 min-h-0 overflow-hidden pwa-main-content">
          ...
          <header className="flex items-center justify-between px-2 sm:px-4 h-11 bg-background border-b border-border flex-shrink-0">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                {activeWorkspace ? (
                  ...
                  <>
                    <span className="font-semibold text-sm">Sim Desk</span>
                    <Badge variant="outline" className="text-[9px] h-4 px-1 font-mono inline-flex">hybrid</Badge>
                  </>
                )}
              </div>
              {currentConvId && (
            ...
            <div className="flex items-center gap-1.5 sm:gap-3">
              ...
              {/* Node bridge and Codex App Server status */}
              <div className="flex items-center gap-1.5">
                ...
                <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-medium ${detected ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : connected ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'bg-muted text-muted-foreground border border-border/30'}`}>
                  ...
                  <span>{detected ? 'Codex' : connected ? 'Codex...' : 'Codex N/A'}</span>
                </div>
              </div>
            </div>
          ...
          {!connected && everDetected && (
            <div className="flex items-center justify-center gap-2 px-3 py-1.5 bg-amber-500/10 border-b border-amber-500/20 text-amber-400 text-xs shrink-0 animate-in slide-in-from-top-1 duration-200">
              ...
              <span>Reconnecting to Sim Desk...</span>
            ...
            <div className="flex-1 flex items-center justify-center">
              {swapping ? (
                <div className="text-center space-y-4 max-w-sm">
                  <div className="flex items-center justify-center gap-3">
                    ...
                    <h2 className="text-xl font-semibold text-foreground/80">Connecting Codex...</h2>
                  </div>
                  <p className="text-sm text-muted-foreground">
                    The local bridge is waiting for the Codex App Server.
                  </p>
                  <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground/70">
                    ...
                    <span>Waiting for Codex CLI...</span>
                  </div>
                </div>
              ) : (
              <div className="text-center space-y-5 max-w-sm">
                <div className="flex items-center justify-center gap-3">
                  ...
                  <h2 className="text-xl font-semibold text-foreground/80">Codex App Server unavailable</h2>
                </div>
                <ol className="text-left space-y-2.5 rounded-lg bg-muted/10 border border-border/30 px-5 py-4">
                  {[
                    'Confirm the Codex CLI is installed and authenticated',
                    'Check that the Node bridge can run codex app-server',
                    'Reconnect, then choose a workspace from the sidebar',
                  ].map((text, i) => (
                ...
                <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground/70">
                  ...
                  <span>Checking Codex App Server...</span>
                </div>
              </div>
              )}
            </div>
          ...
          <footer className="flex items-center justify-between px-2 sm:px-4 h-8 bg-background border-t border-border flex-shrink-0 text-[10px] text-muted-foreground/60">
            <div className="flex items-center gap-2 sm:gap-3">
              <span><FolderSync className="w-3 h-3 inline-block mr-1" />Sim Desk</span>
              <span className="w-px h-3 bg-border hidden sm:block" />
              <span className="hidden sm:inline">Codex CLI workspace</span>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
