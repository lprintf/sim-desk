...
self.addEventListener('fetch', (event) => {
  ...
  // Always prefer the current HTML. This prevents an installed PWA from
  // rendering an old app shell after a Deck upgrade.
  ...
  const isRuntimeAsset = url.pathname.startsWith('/_next/data/') ||
  ...
  if (isRuntimeAsset || isStaticAsset) {
    event.respondWith(
      caches.open(CACHE_NAME).then((cache) =>
        cache.match(event.request).then((cached) => {
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    data = { title: 'Sim Desk', body: event.data?.text() || 'New notification' };
  }

  const title = data.title || 'Sim Desk';
  const options = {
    body: data.body || '',
    icon: '/favicon.ico',
    badge: '/favicon.ico',
