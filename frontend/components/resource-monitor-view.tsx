...
import type { ResourceSnapshot, WorkspaceResources } from "@/lib/cascade-api"
import { Cpu, Activity, Monitor, HardDrive, Server, Box, Terminal, X, AlertTriangle } from "lucide-react"
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
export function ResourceMonitorView() {
    ...
    const workspaceList = useMemo(() =>
        Object.entries(snapshot?.workspaces || {}).sort((a, b) => b[1].memMB - a[1].memMB),
        [snapshot?.workspaces]
    )
    ...
    const cpuHistory = useMemo(() => (snapshot?.history || []).map(h => h.cpu), [snapshot?.history])
    const memHistory = useMemo(() => (snapshot?.history || []).map(h => h.mem), [snapshot?.history])
    ...
    return (
        <div className="flex-1 overflow-y-auto">
            <div className="max-w-3xl mx-auto p-4 sm:p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 justify-items-center">
                        ...
                        <div className="relative">
                            <DonutChart
                                ...
                                label="Agent Share"
                                sublabel={`${workspaceList.length} process${workspaceList.length !== 1 ? 'es' : ''}`}
                                color="hsl(262, 80%, 55%)"
                            />
                        </div>
                    </div>
                ...
                {/* Sim Desk self monitoring */}
                {deckStats && (
                    <div className="rounded-xl border border-border/50 bg-card/50 p-5">
                        <h3 className="text-sm font-medium text-muted-foreground mb-3 flex items-center gap-2">
                            <Box className="w-4 h-4" /> Sim Desk
                        </h3>
                        {/* Column headers */}
                        ...
                        <div className="divide-y divide-border/30">
                            ...
                            <div className="grid grid-cols-[1fr_100px_100px_60px] items-center gap-3 py-2 px-3 rounded-lg hover:bg-muted/30 transition-colors">
                                <div className="flex items-center gap-2 min-w-0">
                                    ...
                                    <span className="text-sm font-medium">Frontend (static)</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-full h-2 rounded-full bg-muted/30 overflow-hidden">
                                        <div className="h-full rounded-full" style={{
                ...
                {/* Agent process breakdown */}
                <div className="rounded-xl border border-border/50 bg-card/50 p-5 overflow-x-auto">
                    <h3 className="text-sm font-medium text-muted-foreground mb-3 flex items-center gap-2">
                        <Cpu className="w-4 h-4" /> Agent Processes
                    </h3>

                    {workspaceList.length === 0 ? (
                        <div className="text-center py-6">
                            <Server className="w-6 h-6 mx-auto text-muted-foreground/30 mb-2" />
                            <p className="text-sm text-muted-foreground/60">No active agent process detected</p>
                        </div>
                    ) : (
                        <>
                            {/* Column headers */}
                            <div className="min-w-[540px] grid grid-cols-[1fr_100px_100px_60px_32px] gap-3 px-3 pb-1 text-[10px] text-muted-foreground/50 uppercase tracking-wider font-medium">
                                <span>Process</span>
                                <span>CPU</span>
                                <span>Memory</span>
                                <span className="text-right">PID</span>
                            ...
                            <div className="min-w-[540px] divide-y divide-border/30">
                                {workspaceList.map(([pid, data]) => (
                                    <WorkspaceRow key={pid} pid={pid} data={data} />
                                ))}
                            </div>
                            {/* Totals */}
                            <div className="min-w-[540px] grid grid-cols-[1fr_100px_100px_60px] gap-3 px-3 pt-2 mt-2 border-t border-border/50 text-xs font-medium">
                                <span className="text-muted-foreground">Total Agent</span>
