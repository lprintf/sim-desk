...
export function AuthGate({ children }: AuthGateProps) {
    ...
    useEffect(() => {
        ...
        // An open local server accepts this request without a key. A protected
        // local server behaves exactly like the tunneled endpoint.
        fetch(`${API_BASE}/api/settings`, {
            headers: savedKey ? { 'X-Auth-Key': savedKey.trim() } : {}
        }).then(res => {
            if (res.ok) {
        ...
        <div className="min-h-dvh w-full bg-background flex items-center justify-center p-4">
            <Card className="w-full max-w-sm">
                <CardHeader className="text-center pb-2">
                    ...
                    <CardTitle className="text-xl">Sim Desk</CardTitle>
