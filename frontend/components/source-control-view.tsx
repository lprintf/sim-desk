...
function TreeNode({
    ...
    }, [type, open, children, workspace, fullPath, onFileClick, showHidden]);
...
function ExplorerTab({ workspace }: { workspace: string }) {
    ...
    useEffect(() => {
        const timer = window.setTimeout(loadRoot, 0);
    ...
    const handleFileClick = useCallback(async (path: string) => {
        ...
        } catch (error: unknown) { setFileError(errorMessage(error)); }
...
export const SourceControlView = memo(function SourceControlView({
    workspace, workspaceLabel, onClose,
